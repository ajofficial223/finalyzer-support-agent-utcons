...
const FormPage = () => {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: "Please set up Google authentication by connecting to Firebase or Clerk.",
      });
  ...
  const simulateSuccessfulAuth = (userData: Omit<FormData, 'password'>) => {
  ...
  const onSubmit = async (formData: FormData) => {
    ...
    try {
      ...
      // For both signin and signup, store user profile and redirect
      const userProfile = {
        name: formData.name,
        email: formData.email,
        industry: formData.industry,
      ...
      <div className="w-full max-w-md">
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="glass-effect rounded-3xl p-6 space-y-5">
            ...
            {/* Industry and organization fields only for signup */}
            {authMode === 'signup' && (
              <>
                <FormField
                  control={form.control}
            ...
            >
              ...
              <span>Sign {authMode === 'signin' ? 'in' : 'up'} with Google</span>
            </Button>
